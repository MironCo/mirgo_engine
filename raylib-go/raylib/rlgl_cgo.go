...
// LoadTextureRaw - Load texture data into GPU memory (VRAM)
// Pass nil for data to create an empty texture (useful for framebuffer attachments)
func LoadTextureRaw(data unsafe.Pointer, width, height, format, mipmapCount int32) uint32 {
	cwidth := C.int(width)
	cheight := C.int(height)
	cformat := C.int(format)
	cmipmapCount := C.int(mipmapCount)
	cid := C.rlLoadTexture(data, cwidth, cheight, cformat, cmipmapCount)
	return uint32(cid)
}
